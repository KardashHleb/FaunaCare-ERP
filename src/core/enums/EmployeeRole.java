 ...
 *    - Основная ответственность: представление ролей сотрудников
 *    - Но также содержит бизнес-логику проверки разрешений (canFeedAnimals, canPerformMedicalProcedures)
 *    - И логику получения ролей по критериям (getRolesForAnimalType, getRolesByAccessLevel)
 *    - Нарушение: смешивает данные с бизнес-логикой контроля доступа
 *    Для enum нарушение SRP в виде объединения данных с их поведением — это не баг, а фича языка
 *
 * 2. OCP (Open/Closed) - ✅ СОБЛЮДЕН
 *    - Закрыт для модификаций существующих ролей
 *    - Открыт для расширения - легко добавить новую роль
 *    - Все методы корректно обрабатывают новые значения через default-case или stream-фильтрацию
 *
 * 3. LSP (Liskov Substitution) - 🔄 НЕ ПРИМЕНИМО
 *    - Перечисление не имеет иерархии наследования
 *    - Все значения enum обрабатываются одинаково
 *    - Нет нарушений принципа подстановки
 *
 * 4. ISP (Interface Segregation) - 🔄 НЕ ПРИМЕНЯЕТСЯ
 *    - Перечисление не реализует интерфейсы
 *    - Предоставляет множество методов, но они все относятся к ролям сотрудников
 *    - Клиенты используют только нужные им методы
 *
 * 5. DIP (Dependency Inversion) - ✅ СОБЛЮДЕН
 *    - Зависит только от другого enum AnimalType (через getRolesForAnimalType)
 *    - Все зависимости явные и стабильные
 *    - Не зависит от конкретных реализаций классов
 */

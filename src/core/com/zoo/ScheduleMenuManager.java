 ...
 *    - Класс отвечает за менеджмент меню расписания животных
 *    - Координирует: выбор животного → отображение меню → выполнение действий
 *    - НО: Также отвечает за UI-логику (вывод, ввод, форматирование)
 *
 * 2. OCP (Open/Closed) - СОБЛЮДЕН:
 *    - Закрыт для модификаций логики меню
 *    - Открыт для расширения через ScheduleService<T>
 *    - Можно добавить новые действия в меню без нарушения OCP
 *
 * 3. LSP (Liskov Substitution) - СОБЛЮДЕН:
 *    - Работает с любым Animal и его наследниками
 *    - Использует обобщенный ScheduleService<Animal>
 *    - Все операции корректны для любого подтипа Animal
 *
 * 4. ISP (Interface Segregation) - СОБЛЮДЕН:
 *    - Использует только необходимые методы ScheduleService
 *    - ScheduleService имеет минимальный контракт для работы с расписанием
 *    - Не зависит от ненужных методов других интерфейсов
 *
 * 5. DIP (Dependency Inversion) - СОБЛЮДЕН:
 *    - Зависит от абстракции ScheduleService<Animal>, а не конкретной реализации
 *    - Scanner и ScheduleService внедряются через конструктор
 *    - Зависит от Database (нарушение) - но только для получения животных
 */
